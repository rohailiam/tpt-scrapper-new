...
async function run(labels) {
  ...
  const scrapingPromises = labels.map(async (label) => {
    ...
    try {
      await page.goto("https://www.teacherspayteachers.com/Browse/Search:" + label, { waitUntil: "domcontentloaded" });
      await page.waitForSelector('.ResultsForSearchResultHeader .Text-module__root--Jk_wf');
      const html = await page.$eval('.ResultsForSearchResultHeader div', (element) => {
